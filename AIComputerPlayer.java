...
public class AIComputerPlayer { 
        ...
        while (flag == false)
        {
            if (rolleta == 0) // this if hold the option to go down from the point.
            {
                if (row + 1 <= rivalBoard.getLengthRow()-1) // it will go down just if the board size allows him.
                {
                    ...
                    if(charSquare == Constants.EMPTY || charSquare == Constants.BATTLESHIP)//בודק אם הנקודה היא לא ירי "אבוד"-כלומר יורה רק אם זה תא ריק או תא שמכיל צוללת(המחשב לא יודע אם זה באמת תא צוללת()
                    { 
            ...
            while((PointIndicator1 == false)||(PointIndicator2 == false)) // the while we finish if it get at least 1 point to the array of 2 points.
            {
                if (PointIndicator1 == false) // enters this if only if there still posibility to get point from down.
                {
                    if (i + 1 <= rivalBoard.getLengthRow()-1) // checks if there no board size problem.
                    {
                        ...
                        else if(charSquare == Constants.EMPTY || charSquare == Constants.BATTLESHIP) //בודק אם הנקודה היא לא ירי "אבוד"-כלומר יורה רק אם זה תא ריק או תא שמכיל צוללת(המחשב לא יודע אם זה באמת תא צוללת()
                        {
    ...
    public void createShootPointBySizeBat(int size)
    {
        ...
        {
            for (int j_right=i; j_right<this.col_board; j_right = j_right+size)//מוסיף את הנקודות מימין לאלכסון הראשי
            {
                if(j_right<this.col_board)//בודק אם הנקודה לא מחוץ ללוח
                {
                    char charSquare = rivalBoard.getSquare(i,j_right) ;
                    if(charSquare == Constants.EMPTY || charSquare == Constants.BATTLESHIP)//בודק אם הנקודה היא לא ירי "אבוד"-כלומר יורה רק אם זה תא ריק או תא שמכיל צוללת(המחשב לא יודע אם זה באמת תא צוללת()
                    {
                        vector_shootPoint.add(new int[] {i,j_right});//מוסיף למערך נקודה אפשרי לירי
                    }
                }
            }
            for (int j_left=i-size; j_left>=min_start_col_board; j_left = j_left-size)//מוסיף את הנקודות שמאל לאלכסון הראשי
            {   //j_left is start from i-size bat, becouse i is alrdey point was add at previous for.
                ...
                if(j_left>=min_start_col_board)//בודק אם הנקודה לא מחוץ ללוח
                {                
                    char charSquare = rivalBoard.getSquare(i,j_left) ;
                    if(charSquare == Constants.EMPTY || charSquare == Constants.BATTLESHIP)//בודק אם הנקודה היא לא ירי "אבוד"-כלומר יורה רק אם זה תא ריק או תא שמכיל צוללת(המחשב לא יודע אם זה באמת תא צוללת()
                    {
                        vector_shootPoint.add(new int[] {i,j_left});//מוסיף למערך נקודה אפשרי לירי
                    }
                }
            }
        }
